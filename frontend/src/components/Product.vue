  ...
  <article class="card bg-base-100 shadow-sm group">
    ...
    <div class="w-full aspect-square bg-base-200 relative overflow-hidden">
      ...
      <button type="button"
        ...
        <img v-show="isImageLoaded" @load="isImageLoaded = true" @error="isImageLoaded = true" :src="currentImage"
          alt="" class="w-full h-full object-cover transition-opacity duration-300" />

        <div v-show="!isImageLoaded" class="skeleton absolute inset-0 w-full h-full z-0" aria-hidden="true"></div>
      </button>
    ...
    <div class="card-body">
      ...
      <div v-if="authStore.isAuthenticated" class="card-actions justify-end mt-2">
        <button type="button" class="btn btn-warning btn-sm" aria-label="Editar producto" @click.stop.prevent="">
          <span class="material-symbols-outlined text-sm" aria-hidden="true">edit</span>
        </button>
        <button type="button" class="btn btn-error btn-sm" aria-label="Borrar producto"
